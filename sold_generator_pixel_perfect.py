...
def generate_sold_image(
    ...
    price, total, currency="GBP",
    badge_scale=1.0, badge_y_offset=0, dot_y_offset=5,
    extra_text_lines=None, extra_text_size=40, extra_text_stroke=2
):
    currency_symbols = {
        "GBP": "£",
        "USD": "$"
    ...
    font_title_first = load_font(FONT_MEDIUM_PATH, 45)
    font_title_rest = load_font(FONT_MEDIUM_PATH, 45)
    font_medium_regular = load_font(FONT_REGULAR_PATH, 45)
    ...
    # BRAND + underline fixed offset below brand
    ...
    brand_bbox = draw.textbbox((brand_x, y_cond), brand, font=font_medium_regular)
    brand_y_bottom = brand_bbox[3] + 3  # 3px below brand text
    draw.line(
        [(brand_x, brand_y_bottom), (brand_x + brand_text_w, brand_y_bottom)],
        fill=COLOR_BRAND, width=2
    )
    ...
    draw.text((sb["padding_left"], y_price), price_text, font=font_price, fill=COLOR_GRAY)

    # TOTAL
    tot_cent = measurements["total_centroid"]
    buyer_text = f"{currency_symbol}{total} Includes Buyer Protection"
    ...
    if extra_text_lines:
        try:
            font_extra = ImageFont.truetype("Montserrat-Bold.ttf", extra_text_size)
        except:
            font_extra = load_font(FONT_MEDIUM_PATH, extra_text_size)

        total_height = len(extra_text_lines) * extra_text_size + (len(extra_text_lines)-1) * 5
        start_y = (h - total_height) // 2

        for line in extra_text_lines:
            if line.strip():
                text_w = draw.textlength(line, font=font_extra)
                x = (w - text_w) // 2
                draw.text(
                    (x, start_y),
                    line,
                    font=font_extra,
                    fill=(255, 255, 255),
                    stroke_width=extra_text_stroke,
                    stroke_fill=(0, 0, 0)
                )
            start_y += extra_text_size + 5
